...
function ensureProjectConfig() {
  ...
  if (!dirents.some((dirent) => dirent.name === 'cljest.edn')) {
    throw new Error(`Could not locate a file named cljest.edn in ${jestProjectDir}.`)
  }
}
...
const getBuildDir = withEnsuredProjectConfig(() => path.resolve(jestProjectDir, '.jest'))
const getJestConfig = withEnsuredProjectConfig(() => require(path.join(jestProjectDir, 'jest.config.js')))
const getCljestConfig = withEnsuredProjectConfig(() => {
  ...
  return parseEDNString(rawCljestConfig, { mapAs: 'object', keywordAs: 'string' })
})
